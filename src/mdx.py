...
SCHEDULER = "euler"         # ddim, ddpm, euler, eulera, lcm, lms, pndm
...
def arg_parser(args):
    ...
    parser.add_argument('-sc', '--scheduler', type = str, default = SCHEDULER, help = f"ddim, ddpm, euler, eulera, lcm, lms, pndm (def: {SCHEDULER})")
...
class MDX():
    ...
    def load_pipeline(self, variant="fp16"):
        ...
        print("Loading checkpoint:", col.CYAN + os.path.basename(self.md.checkpoint) + col.END)
    ...
    def inference(self, pipe):
        ...
        if image:
            ...
            print("Load image:", col.CYAN + self.md.image + col.END)
        if mask:
            mask = Image.open(mask).convert("RGB")
            print("Load mask:", col.CYAN + self.md.mask + col.END)
